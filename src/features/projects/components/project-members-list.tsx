...
const roleBadgeColor: Record<EffectiveProjectAccessRoleType, string> = {
  ...
  NO_ACCESS: 'bg-slate-200/50 text-slate-500 border-slate-300/50 dark:bg-slate-800/50 dark:text-slate-400 hover:bg-slate-200/80',
}
...
export function ProjectAccessManagement({ workspaceId, projectId }: ProjectAccessManagementProps) {
  ...
  if (membersQuery.isLoading || accessListQuery.isLoading || projectQuery.isLoading || teamsQuery.isLoading || workspaceQuery.isLoading) {
    return (
  ...
  const getEffectiveRole = (userId: string) => {
    ...
    return Object.keys(roleWeight).find(k => roleWeight[k as keyof typeof roleWeight] === effectiveWeight) as EffectiveProjectAccessRoleType
  }
            ...
            <TabsList className="h-10 bg-muted/50 p-1 rounded-xl gap-1 w-max">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px] bg-muted-foreground/10">{filteredMembers.length}</Badge>
                </div>
              </TabsTrigger>
              <TabsTrigger 
                value="teams" 
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px] bg-muted-foreground/10">{filteredTeams.length}</Badge>
                </div>
              </TabsTrigger>
            </TabsList>
                ...
                <div className="divide-y divide-border/40 p-2">
                  {filteredMembers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center text-muted-foreground">
                      ...
                      <p className="text-sm font-medium mt-1">{t('project.settings.tryDifferentSearch', { defaultValue: 'Thử tìm với từ khóa khác' })}</p>
                    </div>
                  ) : (
                          ...
                          <div className="flex items-center gap-4 mb-4 sm:mb-0">
                            ...
                            <div className="flex flex-col gap-0.5">
                              <div className="flex items-center gap-2.5">
                                <span className="text-[15px] font-bold tracking-tight text-foreground">{member.user.firstName} {member.user.lastName}</span>
                                {isWorkspaceOwner && (
                                  <Tooltip>
                                    ...
                                    <TooltipContent className="font-bold text-xs">{t('project.settings.workspaceOwner')}</TooltipContent>
                                  </Tooltip>
                                )}
                              </div>
                          ...
                          <div className="flex items-center justify-end w-full sm:w-auto">
                            ...
                            <div className="flex items-center gap-2">
                              {canManageProjectAccess && !isWorkspaceOwner ? (
                                <Select
                                  value={directGrant?.role ?? (project.visibility === 'PUBLIC' ? 'CONTRIBUTOR' : 'NO_ACCESS')}
                                  onValueChange={(val) => {
                                    if (val === 'NO_ACCESS') {
                                    ...
                                    <div className="flex items-center justify-center gap-2 truncate w-full">
                                      <span className="truncate">
                                        {!directGrant && project.visibility !== 'PUBLIC' ? (
                                          <div className="flex items-center gap-2 text-muted-foreground"><Lock className="size-4"/>{t('project.role.no_access')}</div>
                                        ) : <SelectValue />}
                                      </span>
                                    </div>
                                  </SelectTrigger>
                                  <SelectContent className="font-medium rounded-xl shadow-xl">
                                    <SelectItem value="NO_ACCESS" className="text-[13px] font-bold text-destructive py-2.5 focus:bg-destructive/10">
                                      <div className="flex items-center gap-2"><Lock className="size-4"/>{t('project.role.no_access')}</div>
                                    </SelectItem>
                                ...
                                <Badge variant="secondary" className={cn("h-9 gap-2 px-3.5 text-[13px] font-bold border shadow-sm", roleBadgeColor[effectiveRole])}>
                ...
                <div className="divide-y divide-border/40 p-2">
                  {filteredTeams.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center text-muted-foreground">
                      ...
                      <p className="text-base font-bold text-foreground">{search ? t('project.settings.noTeamsResults') : t('workspace.teams.empty')}</p>
                      {search && <p className="text-sm font-medium mt-1">{t('project.settings.tryDifferentSearch', { defaultValue: 'Thử tìm với từ khóa khác' })}</p>}
                    </div>
                  ) : (
                          ...
                          <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto">
                            {canManageProjectAccess ? (
                              ...
                              >
                                <SelectTrigger className={cn(
                                  "h-10 w-[180px] border border-border/50 shadow-sm text-[13px] font-bold transition-all focus:ring-primary focus:ring-offset-0",
                                  directGrant ? roleBadgeColor[directGrant.role as EffectiveProjectAccessRoleType] : 'bg-muted/50 text-muted-foreground hover:bg-muted/80'
                                )}>
                                  <div className="flex items-center justify-center gap-2 truncate w-full">
                                    <span className="truncate">
                                      {!directGrant ? (
                              ...
                              directGrant && (
                                <Badge variant="secondary" className={cn("h-9 gap-2 px-3.5 text-[13px] font-bold border shadow-sm", roleBadgeColor[directGrant.role as EffectiveProjectAccessRoleType])}>
                                  <RoleIcon className="size-4" />
